    ...
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-sm shadow-md' : 'bg-white shadow-md'}`}>
      <nav className="container mx-auto px-4 py-3 flex flex-col xl:flex-row items-center justify-between">
        ...
        <div className="hidden xl:flex items-center gap-1 2xl:gap-3">
          <button onClick={() => scrollToSection('inicio')} className="px-2 2xl:px-3 py-2 text-sm 2xl:text-base text-green-800 hover:text-orange-600 font-medium transition duration-300 whitespace-nowrap">Inicio</button>
          <button onClick={() => scrollToSection('quienes-somos')} className="px-2 2xl:px-3 py-2 text-sm 2xl:text-base text-green-800 hover:text-orange-600 font-medium transition duration-300 whitespace-nowrap">Quienes Somos</button>
          <button onClick={() => scrollToSection('servicios')} className="px-2 2xl:px-3 py-2 text-sm 2xl:text-base text-green-800 hover:text-orange-600 font-medium transition duration-300 whitespace-nowrap">Servicios</button>
          <Link href="/formacion" className="px-2 2xl:px-3 py-2 text-sm 2xl:text-base text-green-800 hover:text-orange-600 font-medium transition duration-300 whitespace-nowrap">Formación</Link>
          <Link href="/catalogo" className="px-2 2xl:px-3 py-2 text-sm 2xl:text-base text-green-800 hover:text-orange-600 font-medium transition duration-300 whitespace-nowrap">Catálogo</Link>
          <button onClick={() => scrollToSection('colabora')} className="px-2 2xl:px-3 py-2 text-sm 2xl:text-base text-green-800 hover:text-orange-600 font-medium transition duration-300 whitespace-nowrap">Colabora</button>
          <button onClick={() => scrollToSection('contacto')} className="px-2 2xl:px-3 py-2 text-sm 2xl:text-base text-green-800 hover:text-orange-600 font-medium transition duration-300 whitespace-nowrap">Contacto</button>
          <button onClick={() => scrollToSection('alta')} className="px-3 2xl:px-4 py-2 text-sm 2xl:text-base bg-amber-500 text-white rounded-full hover:bg-amber-600 transition duration-300 whitespace-nowrap">Darse de Alta</button>
          <a href="https://app.recomenzar.es" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3 2xl:px-4 py-2 text-sm 2xl:text-base bg-green-700 text-white rounded-full hover:bg-green-800 transition duration-300 whitespace-nowrap">Accede a la App <ExternalLink size={14} /></a>
        </div>

        {/* Mobile Menu */}
        <div className={`xl:hidden w-full transition-all duration-300 overflow-hidden ${isMenuOpen ? 'max-h-screen opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
          <div className="flex flex-col space-y-2 pb-4 text-center">
            ...
            <Link href="/catalogo" className="px-3 py-2 text-green-800 hover:text-orange-600 font-medium transition duration-300 block">Catálogo</Link>
